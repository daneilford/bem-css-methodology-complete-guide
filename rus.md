...
#### 1.1 Зачем вообще нужны методологии
Почему недостаточно просто писать CSS?
**Ответ — писать на CSS легко, сложно поддерживать и развивать код.**

Легко заменять разработчиков! :)
Не имеет значения - у вас один большой портал с общими правилами, или куча независимых проектов: правила и огранизация работы нужна везде и это принесет пользу на каждом проекте. Описывать всё блоками можно и на маленьких сайтах, не обязательно у вас должен быть большой портал, где нужна библиотека блоков. Декомпозиция - это просто удобно.
Нет проблемы ухода сотрудника в отпуск или непонятности его кода.
Даже лендинги удобно верстать по БЭМ - меньше думаешь над именами классов, нет конфликтов, легко поддерживать, расширять.
Когда вы делаете правки в чужом спагетти-коде - вы можете использовать БЭМ - вам будет легче работать, не будет конфликтов кода, и постеменно произойдёт рефакторинг.
...
##### Разновидности BEM:
 ...
 - bem-bl,
 - Full Stack BEM.
 - сторонние, не-Яндекс-реализации.
Всё это BEM.**
Разновидности называются "подмножествами или диалектами BEM".
БЭМ может быть реализован в разных технологиях: просто css, в sass, в bemjson, в jade…

##### Что не является БЭМ.
НЕ БЭМ:
- БЭМ-оподобный синтаксис где нельзя выделить блок, элемент и модификатор (твит Харисова, пример редизайна Vk)
...
SMACSS, MCSS, SUIT, Systematic CSS, ECSS, FUN… — это просто вариации БЭМ.
...
OOCSS хотел добавить в css логику, сделать что-то похожее на объектно-ориентированное программирование, но реально ООП в css удалось реализовать не в OOCSS, а именно в БЭМ.
...
##.block
![.block](http://ihorzenich.github.io/talks/webcamp/2015/bem/pictures/block.png)
...
##.block__element
![.block__element](http://ihorzenich.github.io/talks/webcamp/2015/bem/pictures/element.png)
...
БЭМ.info, [Методология](https://ru.bem.info/method/definitions)
...
![block__el__el](http://ihorzenich.github.io/talks/webcamp/2015/bem/pictures/block__el__el.png)
...
Но для всего мира ([csswizardry](http://csswizardry.com/2013/01/mindBEMding-getting-your-head-round-БЭМ-syntax/),[cssguidelin](http://cssguidelin.es/#BEM-like-naming),[sitepoint](http://www.sitepoint.com/bem-smacss-advice-from-developers/)) это:
```css
.blockname__elem--mod {}
```
...
В том числе упор на
**Повторение DOM-дерева**
**Привязка к блоку-родителю**
https://twitter.com/delaz/status/580353955244482560
Примеры разбора от Prom.ua: https://dl.dropboxusercontent.com/u/66873654/%D0%91%D0%AD%D0%9C%D0%BE%D1%81%D0%B8%D0%BF%D0%B5%D0%B4.pdf
...
[1]: http://delka.name/blog/2013/04/bem-otkroveniya-prinyavshih-veru/
[2]: http://habrahabr.ru/post/203440/
[3]: http://clubs.ya.ru/bem/replies.xml?item_no=4
